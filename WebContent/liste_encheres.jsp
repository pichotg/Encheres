...
<h1>ENI - Enchères</h1>
	...
	<p><a href="<%=request.getContextPath()%>/creerVente.jsp">Vendre Article</a></p>
	</c:if>
	...
	<c:forEach var="enchere" items="${listeEncheres}">
		...
		<div class="enchere">
			...
			<label for="vendeur" >Vendeur : <a target ="_blank" name ="modifier" value="disabled" href="<%=request.getContextPath()%>/profil?id_utilisateur_recherche=${enchere.noArticle.utilisateur.noUtilisateur}">${enchere.noArticle.utilisateur.pseudo}</a></label><br/>
		</div>
		</section>
	</c:forEach>
	</section>
