...
/****** Object:  StoredProcedure [dbo].[liste_filtre_categorie_nom_connecte]]    Script Date: 07/05/2019 10:15:27 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[liste_filtre_categorie_nom_connecte] @no_categorie integer, @contient varchar (30)
AS 
IF @no_categorie != 5 AND @no_categorie is not null
	...
	left join EN_COURS_CTE ec on ec.no_article = av.no_article
	left join ENCHERES e on e.no_article = av.no_article and e.montant_enchere = ec.enchereMax
	WHERE av.no_categorie = @no_categorie AND av.nom_article like @contient
	order by av.date_fin_encheres ASC;
